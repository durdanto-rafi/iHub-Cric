...
import static com.intellehub.extras.Keys.EndpointNews.KEY_LATEST_NEWS;
/**
...
public class Parser
{
    public static ArrayList<LatestNews> parseLatestNewsJson(JSONObject response)
    {
        ...
        ArrayList<LatestNews> listMovies = new ArrayList<>();
        if (response != null && response.length() > 0)
        {
            try
            {
                JSONArray arrayMovies = response.getJSONArray(KEY_LATEST_NEWS);
                for (int i = 0; i < arrayMovies.length(); i++)
                {
                    long id = -1;
                    String title = SyncStateContract.Constants.NA;
